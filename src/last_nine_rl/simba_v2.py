...
class SimbaNormalTanhActor(nn.Module):
    def __init__(
        ...
        self.log_std_floor = (
            None
            if cfg.simba_actor_log_std_floor is None
            else float(cfg.simba_actor_log_std_floor)
        )
    ...
    def forward_with_unclamped_log_std(
        self, x: torch.Tensor
        ...
        unclamped_log_std = SIMBA_LOG_STD_MIN + 0.5 * (
            SIMBA_LOG_STD_MAX - SIMBA_LOG_STD_MIN
        ) * (log_std + 1.0)
        clamped_log_std = (
            unclamped_log_std
            if self.log_std_floor is None
            else torch.clamp(unclamped_log_std, min=self.log_std_floor)
        )
        return mean, clamped_log_std, unclamped_log_std

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        mean, log_std, _unclamped_log_std = self.forward_with_unclamped_log_std(x)
        return mean, log_std
