...
We trained five independent SimbaV2 actor-critic agents for 100,000 environment transitions using reward-based SAC without reference labels. The actors use a single residual block with width 32. The twin categorical critics use two residual blocks, width 64, and 51 return atoms \cite{bellemare2017distributional}.

Standard SAC bootstraps after one step, whereas SACN8 calculates losses across horizons one through eight. FastSACN8 retains only the one-step and eight-step losses, reducing target computation. The selected recipe weights the eight-step loss by $\lambda^7$ and updates each critic twice per transition. At deployment, reflection enforces Pendulum sign symmetry. Q-search proposes alternative actions, but a searched action replaces the actor proposal only when both critics predict a higher value.

The actor minimizes the SAC objective $\mathbb E[\alpha\log\pi_\theta(a|s)-\min_iQ_i(s,a)]$. For the critics, let $Y_t^{(n)}$ be the $n$-step soft return and let $Z_t^{(n)}$ be its categorical projection. For $n\in\{1,8\}$,
\[
\begin{aligned}
...
If $\operatorname{CE}(Z,Q_i)$ denotes the cross-entropy between the projected target distribution and critic $i$, FastSACN8 minimizes
\[
\mathcal L_Q=
\frac{1}{1+\lambda^7}
...
We evaluated seeds 0 through 4. Adam uses batch 256, replay capacity 100k, discount $\gamma=.99$, and soft-update rate $\tau=.005$. The actor and critic learning rates decay linearly from $10^{-4}$ to $5{\times}10^{-5}$. After learning starts, the critic receives two updates and the actor one update per environment transition.

At deployment, the symmetric state mapping $M(s)$ and symmetrized action $a_{\rm sym}(s)$ are
\[
\begin{aligned}
...
Reflection imposes Pendulum sign symmetry. Q-search evaluates 41 evenly spaced torques in $[-2,2]$ using the learned reward critics. The highest-scoring candidate $a_q=\arg\max_a\min_iQ_i(s,a)$ is accepted only when both critics predict an advantage over the symmetrized action exceeding 0.005:
\[
...
\smallskip
\noindent\textbf{Use of generative AI.} Generative AI was used to polish the writing and correct grammatical errors.
\end{multicols}
