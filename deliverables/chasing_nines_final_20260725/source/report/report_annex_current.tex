...
\captionof{figure}{Normalized horizon weights for FastSACN target variants. The $\lambda=0.5$ FastSACN8 recipe assigns 0.775\% of normalized target mass to the eight-step endpoint.}
\end{center}
...
\begin{center}
\includegraphics[width=.96\textwidth]{25_terminal_fastsacn_gate.png}
\captionof{figure}{Five-seed locked off-grid comparison under the same reflection and Q-search deployment. P7 changes both the FastSACN8 target and critic update dose. The authority grid was not queried.}
\end{center}
...
The separation between generated experience and the policy extracted from it follows exploration-versus-optimization diagnostics \cite{berseth2025exploration}. The registered threshold finds zero dormant units in the completed SimbaV2 arms. This is a negative result. It rules out widespread measured dormancy as the explanation for the selected reliability gap. It does not establish healthy plasticity, well-conditioned gradients, or good critic calibration. Effective rank, plasticity, and policy churn are narrower complementary checks \cite{gulcehre2022rank,nauman2024plasticity,tang2024churn}. Loss magnitudes also differ across target objectives and should not be ranked without normalization.
...
\begin{center}
\includegraphics[width=.95\textwidth]{09_sequence_control_diagnostic.png}
\captionof{figure}{Repair rate as a state-matched reference policy controls the first 1, 8, 16, or 32 steps and then returns control to the same frozen actor.}
\end{center}

\begin{center}
\includegraphics[width=.92\textwidth]{10_reference_prefix_specificity.png}
\captionof{figure}{Specificity controls at 1, 8, 16, and 32 steps. Shuffled reference actions and zero torque do not reproduce the state-matched repair at longer prefixes.}
\end{center}
