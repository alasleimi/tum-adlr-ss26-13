...
SUMMARY_PATHS = {
    ...
    / "reports/canonical_reference_dagger_100k_5seed_20260716/relative/relative_summary.json",
    "DAgger 50k +\nSimbaV2 50k": REPO_ROOT
    / "reports/canonical_reference_dagger50k_simbav250k_5seed_20260716/relative/relative_summary.json",
    "Standard SimbaV2\n100k": REPO_ROOT
...
def main() -> None:
    ...
    static_distill = older["Static distill 1 seed"]
    write_primary_metrics(summaries, static_distill, q_filtered)
    write_per_seed_metrics(summaries)


def write_primary_metrics(
    summaries: dict[str, dict], static_distill: dict, q_filtered: dict
) -> None:
    labels = [
        *summaries,
        "Static distill\n400k (1 seed)",
    ...
    ] + [static_distill["near_best_known_return_eps"], q_filtered["near_best_known_return_eps"]]
    task = [summary["criteria"]["task_success"]["rate"] for summary in summaries.values()] + [
        static_distill["task_success"],
    ...
    axis.set_ylabel("Grid success rate")
