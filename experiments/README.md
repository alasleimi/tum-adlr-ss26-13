...
```bash
uv run python experiments/run_pure_report_matrix.py \
  --family pure_selected --seeds 0 1 2 3 4 --device cuda
```
...
```bash
uv run python experiments/run_mixed_report_pipeline.py initializer \
  --seed 0 --device cuda
```
...
```bash
uv run python experiments/run_mixed_report_pipeline.py selected \
  --seed 0 --device cuda
```
...
Use `uv run last-nine verify` to validate the frozen evidence. Verification and
figure rendering are inexpensive; retraining all report families is not.
