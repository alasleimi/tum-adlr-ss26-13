...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--local-symmetric-actor-fallback",
        action="store_true",
        help="Center local candidates on the reflection-averaged actor.",
    )
    parser.add_argument(
        "--include-reflection",
        action="store_true",
    ...
    for radius in args.local_radii:
        for num_actions in args.local_num_actions:
            for margin in args.local_margins:
                actor_suffix = (
                    "_symmetric_actor"
                    if args.local_symmetric_actor_fallback
                    else ""
                )
                specs.append(
                    {
                        "kind": "local",
                        "name": (
                            f"local_r{radius:g}_n{num_actions}_m{margin:g}"
                            f"{actor_suffix}_unanimous"
                        ),
                        "radius": float(radius),
                        "num_actions": int(num_actions),
                        "margin": float(margin),
                        "filter_mode": "unanimous_advantage",
                        "symmetric_actor_fallback": bool(
                            args.local_symmetric_actor_fallback
                        ),
                    }
                )
    return specs
...
def make_policy(
    ...
    return FixedLocalCriticQSearchPolicy(
        ...
        symmetric_actor_fallback=bool(
            spec.get("symmetric_actor_fallback", False)
        ),
    )
...
def main() -> None:
    ...
    payload = {
        "protocol": {
            ...
            "local_symmetric_actor_fallback": bool(
                args.local_symmetric_actor_fallback
            ),
            "selection_metrics": (
                [
